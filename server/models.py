...
class Power(db.Model):
    ...
    def description_validation(self, key, description):  
        if  description == "":
            raise ValueError('Power  have a description')
        if len(description) < 20:
            raise ValueError('Description should be more than 20 characters')
...
class HeroPower(db.Model):
    ...
    def strength_validation(self, key, strength):  
        if strength not in ['Strong', 'Weak', 'Average']:
            raise ValueError(
                "Strength should be either Strong, Weak, or Average")
        return strength